...
#filename_vcf <- "final_final_NO_maf_merged_soaradeMS_DPfiltered_inds_less_50missing" # tag for the file with genotypes (filename_vcf.GT), chromosome position info (filename_vcf.CHRMPOS), etc.
#indpopinfofilename <- "popmap_no_out_soaradeMS_50missing_carol_pyrnorte_almargem.txt" # filename of the file with IndPopInfo (1st column indID, 2nd column popID)
...
#ind_threshold_s <- "3,3,3" # number of diploid individuals per pop (P1, P2, P3)
...
#seed <- 6126151  # seed for random number generator
...
indpopinfo <- read.table(indpopinfofilename, stringsAsFactors = FALSE, header=FALSE)  
