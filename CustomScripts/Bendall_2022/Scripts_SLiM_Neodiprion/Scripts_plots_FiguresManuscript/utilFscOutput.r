# Definition of functions used to process fastsimcoal2 SFS-based estimates
...
plot2dSFS_single <- function(obsSFS, xtag, ytag, maintext, minentry) {
  ...
  image(0:(nrow(obsSFS)-1), 0:(ncol(obsSFS)-1), log10(obsSFS), col=nclasses, breaks=breaksplot, xlab=xtag, ylab=ytag, main=maintext, cex.lab=1.5, cex.axis=1.5, cex.main=1.5)
...
plot2dSFS <- function(obsSFS,expSFS,xtag,ytag, minentry) {
  ...
  expSFS_counts <- expSFS*sum(obsSFS)
...
#' PLOT_RELDIFF2DSFS_RATIO
#' Plot the fit of the 2D SFS in terms of difference between the observed and expected SFS
#' INPUT
#' @param obsSFS  (selection) matrix with observed SFS with counts of SNPs
#' @param expSFS  (neutral) matrix with expected SFS with counts of SNPs
#' @param xtag  string with the label of x-axis
#' @param ytag  string with the label of y-axis
#' @param marginal  boolean. If TRUE then the entries for monomorphic sites are not set to zero.
#' OUTPUT
#' @return  plot with the relative observed/expected SFS
plot_relDiff2dSFS <- function(obsSFS,expSFS,xtag,ytag, marginal=TRUE) {
  layout(matrix(1:2, nrow = 1), widths = c(0.7,0.3))
  library(RColorBrewer)
  if(!marginal) {
    obsSFS[1,1] <- 0
    expSFS[1,1] <- 0
    obsSFS[nrow(obsSFS),ncol(obsSFS)] <- 0
    expSFS[nrow(expSFS),ncol(expSFS)] <- 0
  }
  # look at the relative
  expSFS <- expSFS/sum(expSFS)
  obsSFS <- obsSFS/sum(obsSFS)
  diffSFS <- expSFS/obsSFS
  nclasses <- rev(c(colorRampPalette(brewer.pal(8,"BrBG"))(15)))
  nclasses[8] <- 0
  breaksplot <- c(seq(min(c(0,diffSFS), na.rm=TRUE),1,length.out = (length(nclasses))/2), seq(1.1,max(c(2,diffSFS), na.rm=TRUE),length.out = (length(nclasses))/2))
  image(0:(nrow(expSFS)-1), 0:(ncol(expSFS)-1), diffSFS, col=nclasses, breaks=breaksplot, xlab=xtag, ylab=ytag, main="Relative difference target/neutral", cex.lab=1.5, cex.axis=1.5, cex.main=1.5)
  image.scale(diffSFS, zlim=range(breaksplot), col = nclasses, breaks=breaksplot, horiz=FALSE, ylab="target SFS/neutral SFS", cex.lab=1.5, cex.axis=1.5, cex.main=1.5)
} 
