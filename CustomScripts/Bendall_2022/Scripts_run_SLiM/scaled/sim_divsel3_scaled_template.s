...
3 { 
	...
	if(CHR == "X") 
	{
		...
		// numgenes is the number of chromosomes in the population
		numgenes = asInteger(round(2.0*Npop*ratioNxNa));
	}
	if(CHR == "A") 
	{
		// numgenes is the number of chromosomes in the population
		numgenes = asInteger(round(2.0*Npop));
	}
	// compute the initial number of genomes to which the mutation under divergent selection will be added
	freqThreshold = asInteger(round(FREQ*numgenes)); // freqThreshold is the number of genomes to which we add the mutation under divergent selection
	...
	gp1 = p1.genomes;
	// index of genomes that are not null
	aux_p1 = !gp1.isNullGenome; 
	// vector of indices of genomes used to sample them
	tmp_p1 = c(0:((p1.individualCount*2)-1)); 
	// sample the index of freqThreshold genomes from population 1
	tmp_m2_sg1 = sample(tmp_p1[aux_p1], freqThreshold, F); 
	// find the genomes without the selected mutation
	diff_sg1=setDifference(tmp_p1[aux_p1],tmp_m2_sg1); 	
	// Add m2 mutations to randomly selected genomes of pop1
	// get the genomes that will get a m2 mut in pop1 (allele a)
	sg1_m2 = gp1[tmp_m2_sg1]; 
	// add m2 mutation at selected position (i.e. positition 250Kb) 
	mutm2 = sg1_m2.addNewDrawnMutation(m2, 249999);  
	// Add m3 mutations to the other genomes of pop1 (allele A)
	// get the genomes that will get a m3 mut in pop1
	sg1_m3 = gp1[diff_sg1]; 
	// add m3 mutation at selected position 
	mutm3 = sg1_m3.addNewDrawnMutation(m3, 249999);  
	...
	gp2 = p2.genomes;
	// index of genomes that are not null
	aux_p2 = !gp2.isNullGenome; 
	// vector of indices of genomes used to sample them
	tmp_p2 = c(0:((p2.individualCount*2)-1));
	// sample the index of freqThreshold genomes
	tmp_m2_sg2 = sample(tmp_p2[aux_p2], freqThreshold, F); 
	// find the genomes without the selected mutation
	diff_sg2=setDifference(tmp_p2[aux_p2],tmp_m2_sg2); 
	// Add m2 mutations to randomly selected genomes of pop2
	// get the genomes that will get a m2 mut in pop2
	sg2_m2 = gp2[tmp_m2_sg2];
	// add m2 mutation at selected position 
	sg2_m2.addMutations(mutm2);  	
	// Add m3 mutations to randomly selected genomes of pop2 (allele A)
	// get the genomes that will get a m3 mut in pop2
	sg2_m3 = gp2[diff_sg2]; 
	// add m3 mutation at selected position 
	sg2_m3.addMutations(mutm3);  
}
