...
#columns 1, 2 and 3 are the names of the two bam files to be merged
#column 4 is the output bam file name
...
# merge the files on column 1 (fileA), column 2 (fileB) and column 3 (fileC) using samtools
# then sort the output by coordinate with samtools
# and save it with the name fileOUT
while read -r fileA fileB fileC fileOUT; 
do
	# merge and save the output to a temporary file
	samtools merge -@ 2 ${BAMFOLDER}/${fileOUT}.bam ${UNMERGED}/sorted_${fileA}.bam ${UNMERGED}/sorted_${fileB}.bam ${UNMERGED}/sorted_${fileC}.bam
	# sort the temporary file and save the output in a final bam file
	samtools sort -@ 2 -o ${BAMFOLDER}/sorted_${fileOUT}.bam ${BAMFOLDER}/${fileOUT}.bam # sort bam file, save in this (-o) file
